...
def preprocess(dataframe, is_train=True, verbose=True):
    if verbose: console.log("Preprocessing data...")
    cat_cols = ['protocol_type', 'service', 'flag']
    num_cols = [col for col in dataframe.columns if col not in cat_cols + ['outcome', 'level']]
    
    # Handle categorical columns
    if is_train:
        if verbose: console.log("Encoding categorical columns...")
        ...
        if verbose: console.log("Loading encoder for categorical columns...")
    ...
    if is_train:
        if verbose: console.log("Scaling numerical columns...")
        scaler = RobustScaler()
        dataframe[num_cols] = scaler.fit_transform(dataframe[num_cols])
        joblib.dump(scaler, "scaler.pkl")
    else:
        if verbose: console.log("Loading scaler for numerical columns...")
...
# Live Sniffer function
def sniff_and_predict(duration=10):
    if not SCAPY_AVAILABLE:
        console.print("[red]scapy is not installed. Please run 'pip install scapy'[/red]")
        return

    console.log(f"[bold cyan]Starting Live Packet Capture for {duration} seconds (requires Administrator/Npcap on Windows)...[/bold cyan]")
    
    flows = defaultdict(lambda: {'duration': 0, 'protocol_type': 'tcp', 'service': 'private', 'flag': 'SF', 'src_bytes': 0, 'dst_bytes': 0, 'count': 0})
    ...
    def packet_callback(packet):
        if IP in packet:
            ...
            flows[flow_key]['protocol_type'] = proto
            flows[flow_key]['src_bytes'] += len(packet)
            flows[flow_key]['count'] += 1
            
            if dst_port == 80 or src_port == 80: flows[flow_key]['service'] = 'http'
            elif dst_port == 443 or src_port == 443: flows[flow_key]['service'] = 'http'
            elif dst_port == 53 or src_port == 53: flows[flow_key]['service'] = 'domain_u'
            elif dst_port == 21 or src_port == 21: flows[flow_key]['service'] = 'ftp'
            elif dst_port == 22 or src_port == 22: flows[flow_key]['service'] = 'ssh'
            else: flows[flow_key]['service'] = 'private'
    ...
    try:
        ...
        console.print(f"[bold red]Live capture failed, possibly missing Npcap or Admin privileges. Error: {e}[/bold red]")
        return
        
    console.log(f"Captured {len(flows)} unique flows.")
    if len(flows) == 0:
        return

    console.log("[cyan]Evaluating live flows...[/cyan]")
    scaler = joblib.load("scaler.pkl")
    encoder = joblib.load("encoder.pkl")
    rf_model = joblib.load("rf_model.pkl")
    ...
    console.log("[cyan]Resolving IP Identities and active Windows Processes...[/cyan]")
        ...
        try:
            for c in psutil.net_connections(kind='inet'):
                if c.laddr and c.laddr.port == port:
                    if c.pid:
                        try:
                            # Use psutil.Process explicitly or pass the pid
    ...
    # Helper function to identify the owner of the IP addresses
    def identify_ip(ip):
        if ip.startswith("192.168.") or ip.startswith("10.") or ip.startswith("127."):
            return "My Device/Router"
        try:
            name = socket.gethostbyaddr(ip)[0]
            # Strip extremely long generic cloud names for neatness
            if ".1e100.net" in name: return "Google Service"
            if ".amazonaws.com" in name: return "AWS Cloud Server"
            if ".fastly.net" in name: return "Fastly CDN"
            return name.split('.')[0] if len(name) > 30 else name
        except Exception:
            return "External Internet Server"

    # Sort flows to bring potential attacks (high count or loopbacks) to the top!
    sorted_flows = sorted(flows.items(), key=lambda x: (x[0][0] == x[0][1], x[1]['count']), reverse=True)

    # Limit to top 10 most relevant flows!
    for key, stats in sorted_flows[:10]:
        
        # If the attack is a flood (high count) or a LAND attack (same IP), trigger DoS features
        is_attack = (key[0] == key[1]) or (stats['count'] > 15)
        simulated_error_rate = 1.0 if is_attack else 0.0
        
        row = {
            'duration': stats['duration'], 'protocol_type': stats['protocol_type'], 'service': stats['service'], 'flag': stats['flag'], 
            'src_bytes': stats['src_bytes'], 'dst_bytes': stats['dst_bytes'], 'land': 1 if key[0] == key[1] else 0, 'wrong_fragment': 0, 'urgent': 0,
            ...
            'count': stats['count'], 'srv_count': stats['count'], 'serror_rate': simulated_error_rate, 'srv_serror_rate': simulated_error_rate, 'rerror_rate': 0.0,
            'srv_rerror_rate': 0.0, 'same_srv_rate': 0.05 if is_attack else 1.0, 'diff_srv_rate': 0.06 if is_attack else 0.0, 'srv_diff_host_rate': 0.0,
            'dst_host_count': 255 if is_attack else 1, 'dst_host_srv_count': 15 if is_attack else 1, 'dst_host_same_srv_rate': 0.05 if is_attack else 1.0, 
            'dst_host_diff_srv_rate': 0.07 if is_attack else 0.0, 'dst_host_same_src_port_rate': 0.0, 
            'dst_host_srv_diff_host_rate': 0.0, 'dst_host_serror_rate': simulated_error_rate, 
            'dst_host_srv_serror_rate': simulated_error_rate, 'dst_host_rerror_rate': 0.0, 
            'dst_host_srv_rerror_rate': 0.0
        }
        test_data = pd.DataFrame([row])
        try:
            test_data = preprocess(test_data, is_train=False, verbose=False)
            rf_prediction = rf_model.predict(test_data)[0]
            rf_label = "Normal" if rf_prediction == 0 else "Attack"
            color = "green" if rf_prediction == 0 else "red"
            
            src_identity = identify_ip(key[0])
            dst_identity = identify_ip(key[1])
            
            # Attach process name if the IP is local
            if "My Device" in src_identity:
                src_identity += get_process_for_port(key[2])
            if "My Device" in dst_identity:
                dst_identity += get_process_for_port(key[3])
            
            console.print(f"Flow [bold]{key[0]}:{key[2]} [{src_identity}] -> {key[1]}:{key[3]} [{dst_identity}][/bold]\n   => ({key[4]}/{stats['service']}) RF Prediction: [{color}]{rf_label}[/]")
        except Exception as e:
            pass
...
if __name__ == "__main__":
    ...
    while True:
        ...
        console.print("Enter a number (1-11):")
        console.print("1 - 10: Predefined scenarios")
        console.print("11: Live Network Traffic Analysis (10s capture)")
